...
use qs::actix::QsQueryConfig;
use qs::Config as QsConfig;
use serde::de::Error;
use qs::actix_validate::ValidateQsQuery;
...
fn test_default_error_handler() {
    futures::executor::block_on(async {
        ...
        let e = ValidateQsQuery::<Query>::from_request(&req, &mut pl)
            .await
            .unwrap_err();
        assert_eq!(
...
#[test]
#[cfg(any(feature = "actix-validator", feature = "actix2-validator"))]
fn test_default_error_handler_validator() {
    futures::executor::block_on(async {
        let req = TestRequest::with_uri("/test").to_srv_request();
        let (req, mut pl) = req.into_parts();
...
fn test_custom_error_handler() {
    futures::executor::block_on(async {
        ...
        let (req, mut pl) = req.into_parts();
        let query = ValidateQsQuery::<Query>::from_request(&req, &mut pl).await;
...
fn test_composite_querystring_extractor() {
    futures::executor::block_on(async {
        ...
        let s = ValidateQsQuery::<Query>::from_request(&req, &mut pl).await.unwrap();
        assert_eq!(s.foo, 1);
